...
export const createPoll = async (
    ...
    if (currentUser.role !== UserRoleEnum.COACH && currentUser.role !== UserRoleEnum.BOSS) {
...
export const removePolls = async (
    ...
    if (polls.length !== ids.length) {
        ...
        return CustomResponse(404, `Some polls not found: ${notFoundIds.join(', ')}`);
    }
    ...
    try {
        ...
        return CustomResponse(200, `${polls.length} poll(s) and associated votes deleted successfully`, true);
    } catch (error) {
