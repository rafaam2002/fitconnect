...
import {
    ...
    FixMessageProps, GetConversationProps,
    GetMonthlyScheduleStats, GetPollProps, GetScheduleProps, GetScheduleRangeProps, GetTrainingTaskProps,
    GetUserWeightsProps, IdProps,
    MessageProps,
    RemoveScheduleProps,
    ...
    ScheduleProps, ScheduleResumeRange, ScheduleStatsProps,
    UnfixMessageProps,
    updateScheduleOptionsProps, UserListProps,
    UserPictureProps,
    UserProps,
...
export const getSchedulesStats = async (
    ...
    } else if (currentUser.role !== UserRoleEnum.BOSS) {
        return CustomResponse(403, "You are not authorized to perform this action");
    }
...
export const updateUser = async (_, args: UserProps, context: ContextProps) => {
    ...
    if (currentUser.id !== userId && currentUser.role !== UserRoleEnum.BOSS) {
        return CustomResponse(403, "You are not authorized to perform this action");
    }
...
export const createMessage = async (
    ...
    if (isFixed && currentUser.role === UserRoleEnum.STANDARD)
        return CustomResponse(403, "You are not authorized to perform this action");
...
export const addUserToSchedule = async (
    ...
    const isUserBoss = currentUser.role === UserRoleEnum.BOSS;
    const isUserCoachOfEvent =
        currentUser.role === UserRoleEnum.COACH && schedule.admin.id === currentUser.id;
    const maxBookings =
...
export const removeUserFromSchedule = async (
    ...
    if (userId) {
        if (
            ...
            (currentUser.role === UserRoleEnum.COACH && userId === schedule.admin.id) ||
            currentUser.role === UserRoleEnum.BOSS
        ) {
            id = userId;
        } else {
            return CustomResponse(
...
export const unfixMessage = async (
    ...
    if (
        currentUser.role === UserRoleEnum.COACH &&
...
export const changeScheduleStatus = async (
    ...
    if (schedule.admin.id !== currentUser.id && currentUser.role !== UserRoleEnum.BOSS)
        return CustomResponse(403, "You are not authorized to perform this action");
