...
import {CreditCardProvider, PaymentType, ScheduleState, SubscriptionStatus, UserRol,} from "../../../types/enums";
...
import {
    ...
    ContextProps, DeletePollProps, FixMessageProps,
    MessageProps, PollProps,
    RemoveSheduleProps,
    ScheduleDevelopmentProps,
    ScheduleProps, UnfixMessageProps,
    UserProps, VoteProps
} from "./types";
...
export const createUser = async (_, args: UserProps, context: ContextProps) => {
    ...
    try {
        ...
        return CustomResponse(200, "User created successfully", true, {user: newUser, token});
    } catch (error) {
...
export const updateUser = async (_, args: UserProps, context: ContextProps) => {
    ...
    try {
        ...
        return CustomResponse(200, "User updated successfully", true, {user: updateUser});
    } catch (error) {
...
export const createMessage = async (_: any, args: MessageProps, context: ContextProps) => {
    const {message} = args;
    const {em, currentUser} = context;
    ...
    if (isFixed && receiverId != process.env.DB_FORUM_ID)
        return CustomResponse(403, "You can only fix messages in the forum", false, {user:null});
    ...
    try {
        ...
        return CustomResponse(200, "Message created successfully", true, {sms: newMessage});
    } catch (error) {
        console.error(error);
...
export const createSchedule = async (_: any, args: ScheduleProps, context: ContextProps) => {
    const {schedule} = args;
    const {em, currentUser} = context
    const {title, description, startDate, endDate, maxUsers, repeatDays} = schedule;
    ...
    if (repeatDays.length > 0) {
        ...
        try {
            return CustomResponse(200, "Schedule created successfully", true, {schedule: newSchedule});
        } catch (error) {
...
export const addUserToSchedule = async (_: any, args: AddScheduleProps, context: ContextProps) => {
...
export const removeUserFromSchedule = async (_: any, args: RemoveSheduleProps, context: ContextProps) => {
    ...
    return CustomResponse(200, "User removed from schedule", true, {schedule: {...schedule, isBooked}});
};

export const createScheduleDevelopment = async (_: any, args: ScheduleDevelopmentProps, context: ContextProps) => {
...
export const createPoll = async (_: any, args: PollProps, context: ContextProps) => {
    const {poll} = args;
    ...
    try {
        ...
        return CustomResponse(200, "Poll created successfully", true, {poll: newPoll});
    } catch (error) {
        console.error(error);
...
export const createOrChangePollVote = async (_: any, args: VoteProps, context: ContextProps) => {
    const {vote} = args;
    ...
    return CustomResponse(200, "Vote created successfully", true, {vote: newPollVote});
};

export const deletePollVote = async (_: any, args: DeletePollProps, context: ContextProps) => {
    const {pollId} = args;
    const {em, currentUser} = context;
...
export const unfixMessage = async (_: any, args: UnfixMessageProps, context: ContextProps) => {
...
export const changeScheduleStatus = async (_: any, args: ChangeScheduleStatusProp, context: ContextProps) => {
    const {scheduleId} = args;
    const {em, currentUser} = context;
    ...
    return CustomResponse(200, "Schedule status changed successfully", true, {schedule});
};

export const createSubscription = async (_: any, args: any, context: ContextProps) => {
    const {paymentMethod, cardId, planId, applePayToken, googlePayToken} = args.subscription;
    const {em, currentUser} = context;
...
export const removeSubscription = async (_: any, args: any, context: ContextProps) => {
    const {planId} = args;
    const {em, currentUser} = context
...
export const addCreditCard = async (_: any, args: any, context: ContextProps) => {
