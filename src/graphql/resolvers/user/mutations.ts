...
const stripe = new Stripe(process.env.STRIPE_SECRET_KEY || 'sk_test_CGGvfNiIPwLXiDwaOfZ3oX6Y', {
...
export const addSubscription = async (_: any, args: any, {em, currentUser}: {
    em: EntityManager,
    currentUser: User
}) => {
    ...
    const paymentData = {
        em, paymentMethod, cardId, planId, subscription, applePayToken, googlePayToken
    }
...
export const removeSubscription = async (_: any, args: any, {em, currentUser}: {
    em: EntityManager,
    currentUser: User
}) => {
...
export const addPayment = async (paymentData: any) => {
    ...
    const {em, paymentMethod, cardId, planId, subscription, applePayToken, googlePayToken, user} = paymentData;
    ...
    try {
        ...
        subscription.endDate = new Date(
            new Date().setMonth(
                new Date().getMonth() + (subscription.plan.paymentType === 'MENSUAL' ? 1 : 12)
            )
        );
