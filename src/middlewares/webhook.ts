...
export const webhookValidationMiddleware = (
  ...
  console.log(
    `Webhook received with signature: ${signature.toString().substring(0, 20)}...`
  );

  next();
};

export const webhookLoggingMiddleware = (
  req: WebhookRequest,
  res: Response,
  next: NextFunction
) => {
  ...
  // Log request
  console.log(
    `[WEBHOOK] ${new Date().toISOString()} - Incoming webhook from IP: ${req.ip}`
  );
  ...
  res.json = function (body) {
    ...
    console.log(
      `[WEBHOOK] ${new Date().toISOString()} - Response: ${res.statusCode} (${duration}ms)`
    );
