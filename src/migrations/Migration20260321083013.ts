...
export class Migration20260321083013 extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      `create table "company_config" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "polls_enabled" boolean not null default true, "products_enabled" boolean not null default true, "chat_enabled" boolean not null default true, "training_enabled" boolean not null default true, constraint "company_config_pkey" primary key ("id"));`
    );
    this.addSql(
    ...
    this.addSql(
      `alter table "company" drop constraint "company_schedule_options_id_foreign";`
    );
    ...
    this.addSql(
      `alter table "schedule_options" add column "booking_cutoff_minutes" int not null default 0;`
    );

    this.addSql(
      `alter table "company" add column "company_config_id" uuid null;`
    );
    this.addSql(
      `alter table "company" add constraint "company_company_config_id_foreign" foreign key ("company_config_id") references "company_config" ("id") on update cascade on delete cascade;`
    );
    this.addSql(
      `alter table "company" add constraint "company_schedule_options_id_foreign" foreign key ("schedule_options_id") references "schedule_options" ("id") on update cascade on delete cascade;`
    );
    this.addSql(
      `alter table "company" add constraint "company_company_config_id_unique" unique ("company_config_id");`
    );

    this.addSql(`alter table "user" alter column "is_blocked" drop default;`);
    this.addSql(
      `alter table "user" alter column "is_blocked" type boolean using ("is_blocked"::boolean);`
    );
    this.addSql(`alter table "user" alter column "is_verified" drop default;`);
    this.addSql(
      `alter table "user" alter column "is_verified" type boolean using ("is_verified"::boolean);`
    );
    ...
    this.addSql(
      `alter table "push_token" add constraint "push_token_user_id_foreign" foreign key ("user_id") references "user" ("id") on update cascade;`
    );
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(
      `alter table "company" drop constraint "company_company_config_id_unique";`
    );

    this.addSql(
      `alter table "company" add constraint "company_schedule_options_id_foreign" foreign key ("schedule_options_id") references "schedule_options" ("id") on update cascade on delete set null;`
    );
    ...
    this.addSql(
      `alter table "user" alter column "is_blocked" type bool using ("is_blocked"::bool);`
    );
    this.addSql(
      `alter table "user" alter column "is_blocked" set default false;`
    );
    this.addSql(
      `alter table "user" alter column "is_verified" type bool using ("is_verified"::bool);`
    );
    this.addSql(
