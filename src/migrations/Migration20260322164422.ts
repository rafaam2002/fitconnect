...
export class Migration20260322164422 extends Migration {
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(
      `create table "member_ship" ("id" uuid not null, "created_at" timestamptz(6) not null, "updated_at" timestamptz(6) not null, "user_id" uuid not null, "company_id" uuid not null, "role" text check ("role" in ('standard', 'admin', 'coach')) not null, constraint "member_ship_pkey" primary key ("id"));`
    );
    this.addSql(
    ...
    this.addSql(
      `create table "user_companies" ("user_id" uuid not null, "company_id" uuid not null, constraint "user_companies_pkey" primary key ("user_id", "company_id"));`
    );
    ...
    this.addSql(
      `alter table "user_companies" add constraint "user_companies_company_id_foreign" foreign key ("company_id") references "company" ("id") on update cascade on delete cascade;`
    );
    this.addSql(
      `alter table "user_companies" add constraint "user_companies_user_id_foreign" foreign key ("user_id") references "user" ("id") on update cascade on delete cascade;`
    );
    ...
    this.addSql(
      `alter table "user" add column "is_admin_verified" bool not null default false;`
    );
    this.addSql(
