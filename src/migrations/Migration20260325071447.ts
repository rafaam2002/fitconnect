...
export class Migration20260325071447 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `alter table "push_token" drop constraint "push_token_user_id_foreign";`
    );
    ...
    this.addSql(
      `alter table "push_token" add constraint "push_token_user_id_foreign" foreign key ("user_id") references "user" ("id") on update cascade on delete cascade;`
    );
  }
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(
      `create table "auth"."sso_domains" ("id" uuid not null, "sso_provider_id" uuid not null, "domain" text not null, "created_at" timestamptz(6) null, "updated_at" timestamptz(6) null, constraint "sso_domains_pkey" primary key ("id"), constraint domain not empty check (char_length(domain) > 0));`
    );
    this.addSql(
    ...
    this.addSql(
      `create table "auth"."sso_providers" ("id" uuid not null, "resource_id" text null, "created_at" timestamptz(6) null, "updated_at" timestamptz(6) null, "disabled" bool null, constraint "sso_providers_pkey" primary key ("id"), constraint resource_id not empty check ((resource_id = NULL::text) OR (char_length(resource_id) > 0)));`
    );
    this.addSql(
    ...
    this.addSql(
      `alter table "permission" add constraint "permission_module_check" check("module" in ('users', 'schedules', 'payments', 'settings', 'promotions', 'chats', 'polls', 'workouts', 'plans', 'products', 'user_weights', 'stats'));`
    );

    this.addSql(
      `alter table "push_token" add constraint "push_token_user_id_foreign" foreign key ("user_id") references "user" ("id") on update cascade on delete no action;`
    );
