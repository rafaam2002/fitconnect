...
export class Migration20260325071447 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`alter table "permission" drop constraint if exists "permission_module_check";`);

    this.addSql(`alter table "refresh_token" drop constraint "refresh_token_user_id_foreign";`);

    this.addSql(`alter table "push_token" drop constraint "push_token_user_id_foreign";`);

    this.addSql(`alter table "permission" add constraint "permission_module_check" check("module" in ('users', 'schedules', 'payments', 'settings', 'promotions', 'chats', 'polls', 'workouts', 'plans', 'products', 'user_weights', 'stats', 'companies', 'subscriptions'));`);
    ...
    this.addSql(`alter table "push_token" add constraint "push_token_user_id_foreign" foreign key ("user_id") references "user" ("id") on update cascade on delete cascade;`);
  }
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`create table "member_ship" ("id" uuid not null, "created_at" timestamptz(6) not null, "updated_at" timestamptz(6) not null, "user_id" uuid not null, "company_id" uuid not null, "role" text check ("role" in ('standard', 'boss', 'premium', 'coach')) not null, constraint "member_ship_pkey" primary key ("id"));`);
    ...
    this.addSql(`create table "notification" ("id" uuid not null, "created_at" timestamptz(6) not null, "updated_at" timestamptz(6) not null, "type" varchar(255) not null, "message" varchar(255) not null, "link" varchar(255) not null, "user_id" uuid not null, "company_id" uuid not null, constraint "notification_pkey" primary key ("id"));`);
    this.addSql(`create index "notification_created_at_index" on "notification" ("created_at");`);

    this.addSql(`create table "auth"."oauth_authorizations" ("id" uuid not null, "authorization_id" text not null, "client_id" uuid not null, "user_id" uuid null, "redirect_uri" text not null, "scope" text not null, "state" text null, "resource" text null, "code_challenge" text null, "code_challenge_method" auth.code_challenge_method null, "response_type" auth.oauth_response_type not null default 'code', "status" auth.oauth_authorization_status not null default 'pending', "authorization_code" text null, "created_at" timestamptz(6) not null default now(), "expires_at" timestamptz(6) not null default (now() + '00:03:00'::interval), "approved_at" timestamptz(6) null, "nonce" text null, constraint "oauth_authorizations_pkey" primary key ("id"), constraint oauth_authorizations_authorization_code_length check (char_length(authorization_code) <= 255), constraint oauth_authorizations_code_challenge_length check (char_length(code_challenge) <= 128), constraint oauth_authorizations_expires_at_future check (expires_at > created_at), constraint oauth_authorizations_expires_at_future check (expires_at > created_at), constraint oauth_authorizations_nonce_length check (char_length(nonce) <= 255), constraint oauth_authorizations_redirect_uri_length check (char_length(redirect_uri) <= 2048), constraint oauth_authorizations_resource_length check (char_length(resource) <= 2048), constraint oauth_authorizations_scope_length check (char_length(scope) <= 4096), constraint oauth_authorizations_state_length check (char_length(state) <= 4096));`);
    ...
    this.addSql(`create table "auth"."oauth_clients" ("id" uuid not null, "client_secret_hash" text null, "registration_type" auth.oauth_registration_type not null, "redirect_uris" text not null, "grant_types" text not null, "client_name" text null, "client_uri" text null, "logo_uri" text null, "created_at" timestamptz(6) not null default now(), "updated_at" timestamptz(6) not null default now(), "deleted_at" timestamptz(6) null, "client_type" auth.oauth_client_type not null default 'confidential', "token_endpoint_auth_method" text check ("token_endpoint_auth_method" in ('client_secret_basic', 'client_secret_post', 'none')) not null, constraint "oauth_clients_pkey" primary key ("id"), constraint oauth_clients_client_name_length check (char_length(client_name) <= 1024), constraint oauth_clients_client_uri_length check (char_length(client_uri) <= 2048), constraint oauth_clients_logo_uri_length check (char_length(logo_uri) <= 2048));`);
    this.addSql(`create index "oauth_clients_deleted_at_idx" on "auth"."oauth_clients" ("deleted_at");`);

    this.addSql(`create table "auth"."oauth_consents" ("id" uuid not null, "user_id" uuid not null, "client_id" uuid not null, "scopes" text not null, "granted_at" timestamptz(6) not null default now(), "revoked_at" timestamptz(6) null, constraint "oauth_consents_pkey" primary key ("id"), constraint oauth_consents_revoked_after_granted check ((revoked_at IS NULL) OR (revoked_at >= granted_at)), constraint oauth_consents_revoked_after_granted check ((revoked_at IS NULL) OR (revoked_at >= granted_at)), constraint oauth_consents_scopes_length check (char_length(scopes) <= 2048), constraint oauth_consents_scopes_not_empty check (char_length(TRIM(BOTH FROM scopes)) > 0));`);
    ...
    this.addSql(`create table "auth"."sso_providers" ("id" uuid not null, "resource_id" text null, "created_at" timestamptz(6) null, "updated_at" timestamptz(6) null, "disabled" bool null, constraint "sso_providers_pkey" primary key ("id"), constraint resource_id not empty check ((resource_id = NULL::text) OR (char_length(resource_id) > 0)));`);
    ...
    this.addSql(`create table "realtime"."subscription" ("id" int8 generated always as identity not null, "subscription_id" uuid not null, "entity" regclass not null, "filters" user_defined_filter[] not null default '{}', "claims" jsonb not null, "claims_role" regrole generated always as realtime.to_regrole((claims ->> 'role'::text)) stored not null, "created_at" timestamp(6) not null default timezone('utc'::text, now()), "action_filter" text check ("action_filter" in ('*', 'INSERT', 'UPDATE', 'DELETE')) null default '*', constraint "pk_subscription" primary key ("id"));`);
    ...
    this.addSql(`create table "auth"."webauthn_challenges" ("id" uuid not null default gen_random_uuid(), "user_id" uuid null, "challenge_type" text check ("challenge_type" in ('signup', 'registration', 'authentication')) not null, "session_data" jsonb not null, "created_at" timestamptz(6) not null default now(), "expires_at" timestamptz(6) not null, constraint "webauthn_challenges_pkey" primary key ("id"));`);
    ...
    this.addSql(`alter table "permission" add constraint "permission_module_check" check("module" in ('users', 'schedules', 'payments', 'settings', 'promotions', 'chats', 'polls', 'workouts', 'plans', 'products', 'user_weights', 'stats'));`);

    this.addSql(`alter table "push_token" add constraint "push_token_user_id_foreign" foreign key ("user_id") references "user" ("id") on update cascade on delete no action;`);

    this.addSql(`alter table "refresh_token" add constraint "refresh_token_user_id_foreign" foreign key ("user_id") references "user" ("id") on update cascade on delete no action;`);
  }
