...
export class Migration20260624064217 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `alter table "notification" add column "title" varchar(255) not null, add column "read" boolean not null default false;`
    );
    this.addSql(
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(
      `create table "auth"."oauth_consents" ("id" uuid not null, "user_id" uuid not null, "client_id" uuid not null, "scopes" text not null, "granted_at" timestamptz(6) not null default now(), "revoked_at" timestamptz(6) null, constraint "oauth_consents_pkey" primary key ("id"), constraint oauth_consents_revoked_after_granted check ((revoked_at IS NULL) OR (revoked_at >= granted_at)), constraint oauth_consents_scopes_length check (char_length(scopes) <= 2048), constraint oauth_consents_scopes_not_empty check (char_length(TRIM(BOTH FROM scopes)) > 0));`
    );
    this.addSql(
    ...
    this.addSql(`drop index "notification_user_id_index";`);
    this.addSql(`drop index "notification_company_id_index";`);
