...
export class InvoiceService extends BaseService {
    ...
    async findByUser(user: UserActivation): Promise<Invoice[]> {
        return this.em.find(Invoice, { user }, {
    ...
    async findByStripeInvoiceId(stripeInvoiceId: string): Promise<Invoice | null> {
        return this.em.findOne(Invoice, { stripeInvoiceId }, {
            populate: ['user', 'subscription', 'transactions']
        });
    }
     ...
     */
    async findByDateRange(startDate: Date, endDate: Date, userId?: string): Promise<Invoice[]> {
    ...
    async syncFromStripe(stripeInvoiceId: string): Promise<Invoice> {
        try {
            ...
            if (typeof stripeInvoice.customer === 'string') {
                user = await this.em.findOne(User, { stripeCustomerId: stripeInvoice.customer });
            } else if (stripeInvoice.customer && typeof stripeInvoice.customer === 'object') {
                user = await this.em.findOne(User, { stripeCustomerId: stripeInvoice.customer.id });
            }

            if (!user) {
                throw new Error(`User not found for Stripe customer: ${stripeInvoice.customer}`);
            }
            ...
            if (stripeInvoice.subscription) {
                ...
                subscription = await this.em.findOne(Subscription, { stripeSubscriptionId: subscriptionId });
            }
            ...
            invoice.dueDate = stripeInvoice.due_date ? new Date(stripeInvoice.due_date * 1000) : undefined;
     ...
     */
    async calculateRevenue(startDate: Date, endDate: Date): Promise<{
        totalRevenue: number;
        paidRevenue: number;
