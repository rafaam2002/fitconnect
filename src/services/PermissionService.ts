...
import { Permission, PermissionModule, PermissionAction } from '../entities/Permission';
...
export class PermissionService extends BaseService {
    ...
    async syncPermissionsFromMetadata(
        ...
        console.log(`Synced ${permissionNames.length} permissions for plan ${planId}`);
    }
    ...
    async getUserActiveSubscriptionInCompany(
        ...
        return await this.em.findOne(Subscription, {
            user: userId,
            company: companyId,
            status: { $in: [SubscriptionStatus.ACTIVE, SubscriptionStatus.TRIALING] }
        }, {
            populate: ['plan', 'plan.planPermissions', 'plan.planPermissions.permission']
        });
    }
    ...
    async getUserActiveSubscriptions(userId: string): Promise<Subscription[]> {
        return await this.em.find(Subscription, {
            user: userId,
            status: { $in: [SubscriptionStatus.ACTIVE, SubscriptionStatus.TRIALING] }
        }, {
            populate: ['plan', 'company', 'plan.planPermissions', 'plan.planPermissions.permission']
        });
    }
    ...
    async userHasAllPermissions(
        ...
        const userPermissions = await this.getUserPermissionsInCompany(userId, companyId);
    ...
    async userHasPermission(
        ...
        if (!companyId) {
            console.warn('userHasPermission called without companyId. Consider using userHasPermissionInCompany.');
            return false;
        }
     ...
     */
    async planHasPermission(planId: string, permissionName: string): Promise<boolean> {
