...
export class PollService extends BaseService {
    ...
    public async removePolls(
        ...
        if (polls.length !== ids.length) {
            ...
            throw new NotFoundError(`Some polls not found: ${notFoundIds.join(', ')}`);
        }
     ...
     */
    public async getAdminPolls(currentUser: CurrentUser): Promise<ServiceResponse> {
        if (!currentUser) {
            throw new UnauthorizedError();
        }
        ...
        return createServiceResponse(200, 'Admin polls fetched successfully', true, {
