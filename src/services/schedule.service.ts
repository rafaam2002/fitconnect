...
export class ScheduleService extends BaseService {
     ...
     */
    public async getSchedulesFromToday(currentUser: CurrentUser): Promise<ServiceResponse> {
        if (!currentUser) {
            throw new UnauthorizedError();
        }
     ...
     */
    public async getSchedulesResume(currentUser: CurrentUser): Promise<ServiceResponse> {
        if (!currentUser) {
            throw new UnauthorizedError();
        }
        ...
        try {
            ...
            return createServiceResponse(200, 'Schedules found', true, {schedulesResume});
        } catch (error: any) {
     ...
     */
    public async getAdminSchedules(currentUser: CurrentUser): Promise<ServiceResponse> {
        if (!currentUser) {
            throw new UnauthorizedError();
        }
        ...
        return createServiceResponse(200, 'Admin schedules fetched successfully', true, {
     ...
     */
    public async getTodaySchedulesResume(currentUser: CurrentUser): Promise<ServiceResponse> {
        if (!currentUser) {
            throw new UnauthorizedError();
        }
     ...
     */
    public async getScheduleOptions(currentUser: CurrentUser): Promise<ServiceResponse> {
        if (!currentUser) {
            throw new UnauthorizedError();
        }
        ...
        try {
            ...
            const schedulesSummaryFirstMonth = Object.entries(
                ...
                const totalRatio = group.reduce(
                    (sum: number, schedule: any) => sum + schedule.users.length / schedule.maxUsers,
                    0
                );
            ...
            // Mes pasado
            const startPastMonth = moment().month(month - 1).startOf('month').toDate();
            const endPastMonth = moment().month(month - 1).endOf('month').toDate();
    ...
    public async createSchedule(
        ...
        try {
            ...
            if (repeatDays.length > 0) {
                ...
                return createServiceResponse(200, 'Schedule created', true, {schedule})
            } else {
    ...
    public async addUserToSchedule(
        ...
        const maxBookings =
            user.schedules.length >= (scheduleOptions?.maxActiveReservations || Infinity);
    ...
    public async changeScheduleStatus(
        ...
        return createServiceResponse(200, 'Schedule status changed successfully', true, {
            schedule,
        });
    }
    ...
    public async updateScheduleOptions(
        ...
        try {
            ...
            if (!scheduleOptions) {
                scheduleOptions = this.em.create(ScheduleOptions, {} as ScheduleOptions);
            }
            ...
            return createServiceResponse(200, 'Schedule options updated successfully', true, {
                scheduleOptions,
            });
        } catch (error: any) {
            if (error instanceof ForbiddenError || error instanceof UnauthorizedError) {
     ...
     */
    private async sendScheduleCancellationNotifications(schedule: Schedule): Promise<void> {
        try {
            ...
            console.error('Error sending schedule cancellation notifications:', error);
