...
export class UserService extends BaseService {
    ...
    public async getUsers(
        ...
        const where = this.buildUserFilter(query, roleFilter, stateFilter, currentUser);
    ...
    public async findUser(id: string): Promise<ServiceResponse> {
        const userRepo = this.em.getRepository(User);
        const user = await userRepo.findOne({id}, {populate: ["pendingCompanies"]});
    ...
    public async updateUserPicture(
        ...
        try {
            ...
            return createServiceResponse(200, "User picture updated successfully", true, {
                user,
            });
        } catch (error) {
    ...
    public async getAdminStats(em: SqlEntityManager, currentUser: CurrentUser): Promise<ServiceResponse> {
        if (!currentUser) {
            throw new UnauthorizedError();
        }
        ...
        const result = await knex("user as u").select([
            ...
            knex.raw("COUNT(CASE WHEN u.is_active = false THEN 1 END) as notactiveusers"),
            knex.raw(
